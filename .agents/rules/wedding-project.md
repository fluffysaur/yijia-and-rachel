...
- Prefer standard numeric scale tokens (e.g. `h-95`, `sm:h-115`, `lg:h-135` where token = px / 4) over arbitrary bracket values (e.g. `h-[380px]`, `sm:h-[460px]`).
...
- Action buttons: use the shared `<Button>` component (`src/components/Button.tsx`) with tracked uppercase styling (`uppercase tracking-[0.14em]`) and clean, concise labels (e.g., "View gallery" instead of "View all 29 photos").
